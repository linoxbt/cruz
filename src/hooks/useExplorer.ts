...
// Generic explorer fetch hook. `path` is a Blockscout v2 path like
// "/transactions" or "/addresses/0x..../logs". CRUZ is single-chain
// (Arbitrum One), so unlike a multi-chain explorer there's no
// network/chainId dispatch here — every call goes to the same backend.
export function useExplorer<T = unknown>(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const res = (await getExplorerData({ data: { path: path as string } })) as
