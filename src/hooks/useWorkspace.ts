...
// Multi-file Contract Editor workspace: a flat localStorage-backed
// {path: content} map plus an explicit folder list (so empty folders
// persist), with a tree view derived on read. Ported from the sibling
// DevStation project's useWorkspace.ts — no DevStation-specific coupling in
// the logic itself, just the storage key and starter content below.

const STORAGE_KEY = "cruz-workspace-v1";
...
function emptyDefault(): Workspace {
  return {
    activePath: "contracts/Demo.sol",
    files: { "contracts/Demo.sol": STARTER_SOL },
    folders: ["contracts"],
  };
}
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed.files && Object.keys(parsed.files).length > 0) {
        return {
          activePath: parsed.activePath ?? "contracts/Demo.sol",
          files: parsed.files,
