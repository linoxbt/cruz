...
import { useEditorChatStore, type ChatSession } from "@/lib/studio/editorChatStore";
...
// Contract Editor's "Code with AI" panel — a lighter, single-file sibling of
// the AI Builder (which generates whole projects): ask questions, get a
// contract written/audited/explained, and optionally drop the result
// straight into the open file. Ported from DevStation's AiChat.tsx.
export function AiChat({ contextLabel, getContext, onUseCode, className, placeholder }: Props) {
  ...
  // Persistent chat sessions (restored from localStorage).
  const sessions = useEditorChatStore((s) => s.sessions);
  const activeId = useEditorChatStore((s) => s.activeId);
  const hydrate = useEditorChatStore((s) => s.hydrate);
  const newSession = useEditorChatStore((s) => s.newSession);
  const setActive = useEditorChatStore((s) => s.setActive);
  const deleteSession = useEditorChatStore((s) => s.deleteSession);
  const setSessionMessages = useEditorChatStore((s) => s.setSessionMessages);
  useEffect(() => {
    hydrate();
  }, [hydrate]);
          ...
          <div className="space-y-2 font-mono text-[11px] text-meta">
            {configured ? (
              ...
              <div className="rounded border border-warning/40 bg-warning/10 p-2.5 text-warning">
                ...
                <Settings className="mb-0.5 inline h-3 w-3" /> settings above to pick a provider and
                paste your key, or use CRUZ&apos;s default AI if this deployment has one configured.
              </div>
            )}
          </div>
        )}
...
// persisted ai-settings store (shared with the AI Builder).
function SettingsPanel({ onClose }: { onClose: () => void }) {
  const s = useAiSettings();
  const preset = AI_PROVIDERS[s.provider];
  // Local key draft so the user explicitly Saves.
  ...
  const proxyStatus = useAiServerStatus();
  const proxyAvailable = proxyStatus.configured;
  ...
  // The AI-source switch: "Provided" uses the server proxy (operator key);
  // "Your key" uses the controls below.
  const sourceSwitch = (
    <div>
