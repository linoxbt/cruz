...
import { getEthPriceHistory } from "@/lib/api/chainPrice.functions";

interface TxPoint {
  date: string;
  // Arbitrum's Blockscout mirror sends "transactions_count" (confirmed
  // live); some other Blockscout instances send "transaction_count" —
  // read either so this doesn't silently break if that ever changes.
...
// Two analytics charts for the explorer dashboard: daily transactions and
// ETH's price, read from Blockscout's /stats/charts endpoints via the proxy.
// Client only (recharts touches the DOM) — wrap usage in <ClientOnly>.
...
function PriceChart() {
  ...
  // Fall back to CoinGecko's own history if Arbitrum's Blockscout instance
  // ever returns empty chart_data (no oracle configured / a transient gap).
  const needsFallback = data != null && explorerPoints.length === 0;
  const { data: history } = useQuery({
    queryKey: ["eth-price-history"],
    queryFn: () => getEthPriceHistory(),
  ...
  const loading = !data || (needsFallback && !history);

  return (
    <Card title="ETH Price (30d)">
      <div className="h-56 px-2 py-3">
        {loading ? (
          <Spinner />
        ) : points.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={points} margin={{ top: 4, right: 8, bottom: 0, left: 8 }}>
              ...
              <Tooltip
                contentStyle={tooltipStyle}
                labelFormatter={shortDate}
                formatter={(v: number) => [`$${v.toFixed(2)}`, "ETH"]}
              />
