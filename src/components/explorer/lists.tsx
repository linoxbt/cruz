...
export function TxFeed({ txs, symbol = "ETH" }: { txs: ExTx[]; symbol?: string }) {
  return (
        ...
        <tbody>
          {blocks.map((b) => (
            <tr key={b.height} className="border-t border-border hover:bg-surface-2/50">
              ...
              <Td className="text-right text-muted-foreground">
                {formatEth(b.rewards?.[0]?.reward ?? "0")}
              </Td>
            </tr>
          ))}
        </tbody>
...
// Dashboard "view all" link to an explorer list page.
export function ViewAll({
  to,
  label,
}: {
  to: "/explorer/txns" | "/explorer/blocks" | "/explorer/tokens";
  label: string;
}) {
  return (
