...
import { searchHistory } from "@/lib/explorer/searchHistory";
import {
  Card,
  StatCard,
  CopyBtn,
...
import { formatEth, formatUnits, timeAgo, withCommas } from "@/lib/explorer/format";
import type {
  ExAddress,
  ExAddrCounters,
  ExTx,
...
function AddressPage() {
  ...
  useEffect(() => {
    if (!addr) return;
    searchHistory.updateName(hash, resolvedName, addr.is_contract);
  }, [addr, hash, resolvedName]);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <h1 className="font-mono text-base font-bold text-foreground">
          {resolvedName || (isContract ? "Contract" : "Address")}
        </h1>
        {addr.is_verified && (
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Balance" value={`${formatEth(addr.coin_balance)} ETH`} />
        {addr.exchange_rate && (
          <StatCard
            label="Value"
            value={`$${withCommas(
          ...
          <tbody>
            {data.items.map((t, i) => (
              <tr key={i} className="border-t border-border hover:bg-surface-2/50">
                ...
                <Td>{t.from ? <AddrLink hash={t.from.hash} name={t.from.name} /> : "-"}</Td>
                <Td>{t.to ? <AddrLink hash={t.to.hash} name={t.to.name} /> : "-"}</Td>
                <Td className="text-right text-foreground">{formatEth(t.value)}</Td>
              </tr>
            ))}
          </tbody>
        </table>
...
type ContractSubTab = "code" | "abi" | "bytecode";
...
function ContractTab({ addr }: { addr: ExAddress }) {
  ...
  // on Arbiscan (CRUZ doesn't run its own source-verification submission flow).
  if (!verified) {
    return (
      <div className="space-y-4">
        <Card title="Contract">
          <div className="space-y-3 px-4 py-5 font-mono text-xs">
            <p className="text-meta">This contract is not verified on Blockscout.</p>
            <p className="text-muted-foreground">
              Verify the source on{" "}
              <a
                href={`${chainConfig(arbitrumOne.id).explorerUrl}/address/${addr.hash}#code`}
                target="_blank"
                rel="noreferrer"
              ...
              to unlock the ABI and compiler details here.
            </p>
          </div>
        </Card>
