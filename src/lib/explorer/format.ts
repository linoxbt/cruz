// Formatting helpers for the Explorer. Blockscout returns wei amounts as
// decimal strings and timestamps as ISO strings.

// Big decimal-string → number of whole units (ETH has 18 decimals). Avoids
...
export function formatEth(raw: string | null | undefined, maxFrac = 6): string {
  return withCommas(formatUnits(raw, 18, maxFrac));
}

// Gas price (wei) → Gwei string, adaptive for very small values.
